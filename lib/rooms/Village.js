...
const PlayerAccount_1 = require("./PlayerAccount");
class Village extends colyseus_1.Room {
    constructor() {
        super(...arguments);
        this.fixedTimeStep = 1000 / 60;
        this.lastCollectTime = {};
        this.COLLECT_COOLDOWN = 250; // 4 times per second (1000ms / 4). Rate limits the collectLoot message from the client
        this.LOOT_LIFETIME = 30000; // 30 seconds in milliseconds
    }
    onCreate(options) {
        // Set autoDispose to false using the accessor
        this.autoDispose = false;
        this.setState(new RoomState_1.MyRoomState());
        const mapData = JSON.parse(fs.readFileSync(path.join(__dirname, "../maps/VillageMap.tmj"), "utf8"));
        ...
        colliders.forEach((collider) => {
            const obstacle = new RoomState_1.Obstacle();
        ...
        for (let i = 0; i < 5; i++) {
            ...
            const monster = new RoomState_1.Monster();
        ...
        this.onMessage("collectLoot", (client, message) => {
            ...
            if (nearestLoot) {
                ...
                if (nearestLoot.type === RoomState_1.LootType.COIN) {
        ...
        this.onMessage("login", (client, message) => {
            if (message.username && typeof message.username === "string") {
                ...
                if (username.length > 0) {
                    ...
                    const playerData = PlayerAccount_1.PlayerAccount.getPlayerData(username);
                    // Update player data in the game
                    const player = this.state.players.get(client.sessionId);
                    if (player) {
                        player.data = playerData;
                        // Store username in registry
                        app_config_1.playerRegistry.set(client.sessionId, username);
    ...
    checkCollision(entity, obstacle) {
        const entitySize = entity instanceof RoomState_1.Loot ? 8 : 16; // smaller size for coins
    ...
    spawnLoot(x, y) {
        ...
        for (let i = 0; i < 5; i++) {
            const loot = new RoomState_1.Loot();
            loot.type = RoomState_1.LootType.COIN;
            loot.x = x;
            loot.y = y;
            loot.value = 1;
    ...
    onJoin(client, options) {
        ...
        if (username) {
            // Get player data from account system
            player.data = PlayerAccount_1.PlayerAccount.getPlayerData(username);
    ...
    onLeave(client, consented) {
        ...
        if (player && username) {
            ...
            PlayerAccount_1.PlayerAccount.updatePlayerData(username, player.data);
        }
