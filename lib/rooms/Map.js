...
class map extends colyseus_1.Room {
    ...
    onCreate(options) {
        ...
        this.onMessage("collectLoot", (client, _) => {
            ...
            const player = this.state.spawnedPlayers.find((player) => player.id === client.sessionId);
            if (!player)
                return;
        ...
        this.setSimulationInterval((deltaTime) => {
            ...
            for (let i = this.state.loot.length - 1; i >= 0; i--) {
                ...
                if (loot.isBeingCollected) {
                    const player = this.state.spawnedPlayers.find((player) => player.id === loot.collectedBy);
                    if (!player) {
    ...
    async transferPlayerToRoom(player, portal) {
        const client = this.clients.find((c) => c.sessionId === player.id);
        if (!client)
            return;
        // Store player data before transfer
        const playerManager = playerManager_1.PlayerManager.getInstance();
        const playerData = {
            id: player.id,
        ...
        try {
            const targetRoom = await colyseus_2.matchMaker.findOneRoomAvailable(portal.targetRoom, {
                playerId: player.id,
            });
            const reservation = await colyseus_2.matchMaker.reserveSeatFor(targetRoom, {
                playerId: player.id,
                fromPortal: true,
                x: portal.targetX,
                y: portal.targetY,
    ...
    onJoin(client, options) {
        ...
        if (options.fromPortal && options.playerId) {
            ...
            if (playerData) {
                ...
                const player = new RoomState_1.SpawnedPlayer(client.sessionId, playerData.username, options.x || 100, options.y || 100, 0, // velocityX
                0, // velocityY
                playerData.experience, playerData.level, 32, // height
                32, // width
        ...
        const player = new RoomState_1.SpawnedPlayer(client.sessionId, "Player", this.state.mapWidth * 0.8, this.state.mapHeight * 0.3, 0, 0, 0, 1, 32, 32, [], // Empty inventory array is fine, constructor will initialize MapSchema
        Date.now(), false, false, []);
        this.state.spawnedPlayers.push(player);
    }
    onLeave(client, consented) {
        const player = this.state.spawnedPlayers.find((p) => p.id === client.sessionId);
        if (!player)
            return;
        ...
        const playerIndex = this.state.spawnedPlayers.findIndex((p) => p.id === client.sessionId);
        if (playerIndex !== -1) {
