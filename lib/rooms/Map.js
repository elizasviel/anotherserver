...
class map extends colyseus_1.Room {
    ...
    async onJoin(client, options, auth) {
        const playerData = await playerData_1.playerDataManager.getPlayerData(auth.username);
        if (!playerData) {
            throw new Error("Player data not found");
        }
        // Create a new spawned player with the persisted data
        const spawnedPlayer = new RoomState_2.SpawnedPlayer(client.sessionId, auth.username, playerData.lastX || 100, // Default spawn position
        playerData.lastY || 100, 0, // velocityX
        0, // velocityY
        playerData.experience, playerData.level, 32, // height
        32, // width
        Date.now(), false, // isAttacking
        true, // isGrounded
        []);
    ...
    onLeave(client) {
        ...
        if (player) {
            ...
            const index = this.state.spawnedPlayers.findIndex((p) => p.id === client.sessionId);
            if (index !== -1) {
