...
class map extends colyseus_1.Room {
    ...
    onCreate(options) {
        ...
        this.monsterSpawnPoints = TiledMapParser_1.TiledMapParser.parseMonsterSpawnPoints(mapData);
        console.log(`Found ${this.monsterSpawnPoints.length} monster spawn points in the map`);
        options.portals.forEach((portal) => {
            this.state.portals.push(new RoomState_1.Portal(portal.id, portal.x, portal.y, portal.width, portal.height, portal.targetRoom, portal.targetX, portal.targetY, portal.isOneWayPlatform));
        });
                ...
                options.monsters.forEach((m) => {
                    ...
                    if (currentCount < m.maxSpawned &&
                        ...
                        if (this.monsterSpawnPoints.length > 0) {
                            ...
                            const randomIndex = Math.floor(Math.random() * this.monsterSpawnPoints.length);
                            ...
                            const monster = new RoomState_1.SpawnedMonster((0, uuid_1.v4)(), m.monsterType, spawnPoint.x, spawnPoint.y - m.monsterType.height / 2);
                            this.state.spawnedMonsters.push(monster);
                            console.log(`Spawned ${m.monsterType.name} at (${spawnPoint.x}, ${spawnPoint.y - m.monsterType.height / 2})`);
                        }
                        else {
                            // Fallback to default spawn location if no spawn points are defined
                            console.warn("No monster spawn points found in map, using default location");
    ...
    */
    ...
    async onJoin(client, options, auth) {
        ...
        0, // lastProcessedTick
        0, // lastDamageTime
        auth.maxHealth, auth.maxHealth, false, // isInvulnerable
        auth.strength, auth.inventory.map((item) => ({
            loot: new RoomState_2.Loot(item.loot.name, item.loot.width, item.loot.height),
                    ...
                    this.state.spawnedMonsters.forEach((monster) => {
                        ...
                        if (dx < attackRange &&
                            ...
                            if (monster.currentHealth <= 0) {
                                ...
                                const requiredXP = Math.floor(100 * Math.pow(1.5, player.level - 1));
    ...
    handleLootCollection(player) {
        ...
        try {
            ...
            if (player.inventory.has(lootId)) {
                ...
                const lootItem = new RoomState_2.Loot(nearestLoot.name, nearestLoot.width, nearestLoot.height);
