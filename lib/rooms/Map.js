...
class map extends colyseus_1.Room {
    ...
    onCreate(options) {
        ...
        this.onMessage("collectLoot", (client) => {
            ...
            const player = this.state.spawnedPlayers.find((p) => p.username === username);
            if (!player) {
    ...
    */
    ...
    handleLootCollection(player) {
        ...
        // Find the closest loot item within range
        ...
        // If we found a loot item to collect, process it
        if (closestLoot) {
            console.log(`MAP: Player ${player.username} collecting loot`);
            // Mark as being collected
            closestLoot.isBeingCollected = true;
            closestLoot.collectedBy = player.username;
            // Add experience to the player
            if (closestLoot.name.includes("Coin")) {
            ...
            setTimeout(() => {
                const index = this.state.spawnedLoot.indexOf(closestLoot);
                if (index !== -1) {
